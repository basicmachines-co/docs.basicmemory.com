...
description: 'Cloud authentication, bidirectional sync, and mount commands for Basic Memory'
icon: 'cloud'
---
...
Basic Memory v0.15.0 provides cloud storage access through remote MCP, bidirectional sync with rclone, and FUSE filesystem mounting.
...
<Steps>
  <Step title="Authenticate">
    ...
    bm cloud login
    ```
    Opens browser for OAuth 2.1 authentication and validates subscription status.
  </Step>

  <Step title="Set up bidirectional sync">
    ```bash
    bm cloud setup
    ```
    Installs rclone, generates S3 credentials, creates local sync directory (`~/basic-memory-cloud-sync`).
  </Step>
...
# → "Device B's version (10:05 AM) selected over Device A (10:00 AM)"
```
...
├── project-a/                     # Cloud projects as directories
│   ├── notes/
│   │   └── meeting-notes.md
│   └── ideas/
├── project-b/
...
# Pull latest changes
bm sync
# → "✓ Sync completed - 3 files downloaded"
