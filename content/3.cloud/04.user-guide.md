...
Want to edit files locally with Obsidian or VS Code? Set up local file sync:
...
This fetches your cloud notes into a local folder. Pull cloud changes down and push local changes up with `bm cloud pull` / `bm cloud push` as you work.
...
2. Run a manual sync: `bm cloud pull --name main`, then `bm cloud push --name main`
3. If a push or pull reports conflicts, resolve them with `--on-conflict`
