...
You deleted a note you needed. A bulk reorganize went sideways. A teammate cleared out the wrong folder. Basic Memory Cloud has two recovery paths — pick the one that matches what you need to recover.
...
```bash
bm cloud restore notes/important.md --snapshot snap_abc123
```
...
```bash
bm cloud restore <project-name>/ --snapshot snap_abc123
```
...
```bash
bm cloud snapshot list
bm cloud snapshot browse snap_abc123 --prefix my-project/
bm cloud restore my-project/ --snapshot snap_abc123
```
...
For ongoing safety, take a manual snapshot before any change you might want to roll back.
