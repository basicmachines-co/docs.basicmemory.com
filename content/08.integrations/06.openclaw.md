...
Installing also enables the plugin, so there's no separate enable step.

Then make it OpenClaw's memory provider by setting the `memory` slot in
...
```json5
{
  plugins: {
    entries: {
      "openclaw-basic-memory": { enabled: true }
    },
    slots: {
...
Verify with `openclaw plugins doctor`, which should report the project it
resolved and no issues.
...
| `projectPath` | `.` (workspace root) | Path to project directory |
...
| `edit_note` | Incremental edits (append, prepend, find/replace, replace section) |
...
| Command | Description |
|---------|-------------|
| `/bm-setup` | Install or update the Basic Memory CLI (requires uv) |
...
| `/notes`, `/ingest`, `/lifecycle`, `/metadata-search`, `/research` | One per remaining bundled skill |
