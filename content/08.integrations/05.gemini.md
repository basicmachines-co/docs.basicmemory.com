...
::steps
### Sign Up
Create your account at [basicmemory.com](https://basicmemory.com/?utm_source=docs&utm_medium=docs&utm_campaign=gemini).
...
### Authenticate
Start `gemini` — your browser opens to authorize Basic Memory Cloud (or run `/mcp auth basic-memory-cloud` inside Gemini).
...
**Gemini can't find Basic Memory** -- Verify uvx can run it (`uvx basic-memory --version`), then check `gemini mcp list`. If the entry looks wrong, remove and re-add it:
```bash
...
**Notes not updating** -- Check file permissions in your knowledge base directory and look for errors in `~/.basic-memory/basic-memory.log`.
