...
Already installed? (If not, start with [Quickstart: Local](/start-here/quickstart-local).) This page covers what comes next: where your notes are saved, working with multiple projects, and keeping Basic Memory updated.
...
Set a default fallback project — it's the same setting the command above writes:
