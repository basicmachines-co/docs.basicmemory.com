...
The plugin bundles the same pre-built skills as the standalone [skills collection](/integrations/skills) — they teach agents best practices for Basic Memory workflows, from note format and task tracking to reflection and defragmentation. See [Agent Skills](/integrations/skills) for the full list.

::note
The bundled skills are identical to the standalone collection — no separate install needed when using the plugin.
::
