...
This sets your fallback project when no explicit project is passed. You can still override by specifying a different project.
...
[Uses schema_infer with entity_type="person"]
...
When semantic search is enabled, you can find notes by meaning rather than exact keywords. This is helpful when you remember the concept but not the exact words used in the note.
...
Semantic search requires installing extras (`pip install 'basic-memory[semantic]'`) and building initial embeddings (`bm reindex --embeddings`). See the [Semantic Search](/concepts/semantic-search) guide for setup and configuration.
