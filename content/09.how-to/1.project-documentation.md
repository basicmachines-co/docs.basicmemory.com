...
**Cloud is a great home for project docs**: browse and edit them in the [web app](/cloud/web-app), give the whole [team](/teams/about) (and their agents) access, and publish any page externally with a [share link](/cloud/shared-notes). Prefer docs living inside the repo? On a local install, point the project at your `docs/` folder instead:
...
Schema fields are satisfied by observations — an ADR with `- [status] accepted` and `- [decision] Use JWT tokens` passes; markdown headings alone don't count (see [Schema System](/concepts/schema-system)). From that point on, every ADR the AI creates follows this structure. You can validate existing ones too:
...
The AI combines the right filters for each: `status` on an ADR is an observation category, so it filters with `categories=["status"]`; tags and note types are frontmatter, filtered with [metadata search](/concepts/metadata-search). You can also use the `tag:` shorthand in searches, like `tag:api` or `tag:security`.
