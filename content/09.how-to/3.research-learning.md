...
```
You: "Create a schema for my research notes. Each one should record
     key findings, sources, open questions, and connections to other
     topics."
```
...
This is where Basic Memory really shines. Once you've built up a collection of research notes, you need to find things -- and not just by exact title.
...
Combine these with semantic search when you need precision. For example, find all notes tagged `battery-technology` that relate to a specific concept. See [Semantic Search](/concepts/semantic-search) for the full query syntax.

### Exploring Connections

Ask your AI to build context around a note to follow its links:
...
- `key-finding` observations for the takeaways
- `source` observations for references
- `open-question` observations for what's unresolved
- Relations linking to related topics

Schemas validate observations, relations, and frontmatter — not markdown headings. With the schema in place (and a [schema-aware assistant](/concepts/schema-system)), every research note comes out in the same format and easy to scan.
