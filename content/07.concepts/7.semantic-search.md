...
SQLite and PostgreSQL both search complete note bodies. Since v0.23.1, PostgreSQL stores long bodies in bounded, overlapping full-text chunks, allowing deep matches and preserving Boolean query behavior across chunk boundaries. The upgrade migration backfills existing notes automatically. A later `bm reindex --search` replaces rows only for the selected project; it does not clear sibling projects that share the same database.
