...
::tip
**Prefer not to install anything?** Basic Memory Cloud connects your AI in about 2 minutes — [sign up at basicmemory.com](https://basicmemory.com/?utm_source=docs&utm_medium=docs&utm_campaign=quickstart-local) and follow the [cloud quickstart](/start-here/quickstart-cloud).
::
...
Add Basic Memory to your `claude_desktop_config.json`:
...
Installed with Homebrew? Use `"command": "basic-memory", "args": ["mcp"]` instead. **Restart Claude Desktop** after editing.

Config file locations per platform and ENOENT fixes: [Claude Desktop → Local Setup](/integrations/claude-desktop#local-setup).
...
Claude will create a Markdown file at `~/basic-memory/My First Note.md`.
...
Done! I've created "My First Note.md" in your basic-memory folder.
...
```bash
# View in terminal
cat ~/basic-memory/My\ First\ Note.md
