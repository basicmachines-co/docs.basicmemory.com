...
`push`/`pull` are additive (never delete on the destination) and abort on conflicts by default, git-style. `sync`/`bisync` are destructive mirrors and are blocked on Team workspaces. See the [Cloud Sync guide](/cloud/cloud-sync) for details.
