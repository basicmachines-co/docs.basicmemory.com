...
`push`/`pull` are additive (never delete on the destination) and abort on conflicts by default, git-style. `sync` is a destructive mirror and is blocked on Team workspaces. See the [Cloud Sync guide](/cloud/cloud-sync) for details.
