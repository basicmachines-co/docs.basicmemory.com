...
`push`/`pull` are additive (never delete on the destination) and abort on conflicts by default, git-style. `sync` is a destructive mirror and is blocked on Team workspaces. In v0.23 it also deletes previously synced files that become ignored by `.bmignore`; preview with `--dry-run`. `prune` performs only that ignore-based remote cleanup and asks for confirmation unless you pass `--yes`. See the [Cloud Sync guide](/cloud/cloud-sync) for details.
