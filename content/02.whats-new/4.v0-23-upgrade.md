...
The FastEmbed provider does not yet apply query/passage prefixes automatically, so asymmetric embedding models — notably the `intfloat/multilingual-e5` family, the strongest local option for multilingual notes — run off-label and retrieval quality collapses ([#1264](https://github.com/basicmachines-co/basic-memory/issues/1264); fix tracked for v0.23.1). Until then, set the literal prefixes yourself:
